# Converted from copied-from-vkhydras-some-edits(1).ipynb
# Notebook magics removed; original agent logic preserved.
...
FOUR_P_TIEBREAK_ENABLED = False
FOUR_P_TIEBREAK_EPS_FRAC = 0.004
FOUR_P_TIEBREAK_EPS_MIN = 2.0
...
LEADER_BASH_BONUS = 8.0
LEADER_BASH_MIN_STEP = 40
FOUR_P_LEADER_FOCUS_ENABLED = True
FOUR_P_LEADER_FOCUS_TURN_MIN = 65
FOUR_P_LEADER_FOCUS_PROD_GAP = 12
...
FRONTIER_GUARD_ENABLED = False
FRONTIER_GUARD_4P_ONLY = True
FRONTIER_GUARD_TURN_MIN = 70
FRONTIER_GUARD_TURN_MAX = 170
FRONTIER_GUARD_ENEMY_DIST = 26.0
FRONTIER_GUARD_BASE = 10
FRONTIER_GUARD_PROD_MULT = 3
FRONTIER_GUARD_FRAC = 0.25

FOUR_P_PRESSURE_FALLBACK_ENABLED = False
FOUR_P_PRESSURE_TURN_MIN = 35
FOUR_P_PRESSURE_MAX_MOVES_BEFORE = 1
FOUR_P_PRESSURE_MIN_AVAIL = 28
FOUR_P_PRESSURE_MAX_TRAVEL = 30
FOUR_P_PRESSURE_LEADER_BONUS = 28.0
FOUR_P_PRESSURE_ENEMY_BONUS = 8.0
...
def search_step_action(world, max_per_source=3, max_actions_to_eval=10,
    ...
    for act in actions[:max_actions_to_eval]:
        ...
        if (
            ...
            tgt = world.planet_by_id.get(act["target_id"])
            if tgt is not None:
                if tgt.owner == world.leader_id:
                    gain += LEADER_BASH_BONUS
                elif tgt.owner not in (-1, world.player):
                    gain -= LEADER_BASH_BONUS * 0.35
        act["score"] = gain
        scored.append(act)
...
def generate_step_actions(world, max_per_source=3):
    ...
    for src in world.my_planets:
        ...
        if avail < MIN_DISPATCH_SHIPS:
            continue
        targets = []
        focus_leader = _four_p_should_focus_leader(world)
        for t in world.planets:
            if t.owner == world.player:
                continue
            if focus_leader and t.owner != world.leader_id:
                continue
...
def _four_p_should_focus_leader(world):
    if not FOUR_P_LEADER_FOCUS_ENABLED or world.is_2p:
        return False
    if world.step < FOUR_P_LEADER_FOCUS_TURN_MIN:
        return False
    if world.leader_id is None or world.leader_id == world.player:
        return False
    leader_prod = world.owner_production.get(world.leader_id, 0)
    return (leader_prod - world.my_prod) >= FOUR_P_LEADER_FOCUS_PROD_GAP
...
def _nearest_targets(src, world, K, max_travel, target_locked):
    ...
    elif (not world.is_2p) and FOUR_P_TIEBREAK_ENABLED and len(candidates) > 1:
        best_d = candidates[0][1]
        eps = max(FOUR_P_TIEBREAK_EPS_MIN, FOUR_P_TIEBREAK_EPS_FRAC * abs(best_d))
        def _k4(kv):
            tgt, weighted_d, _raw = kv
            bucket = int(weighted_d / eps) if eps > 0 else 0
            return (bucket, _tiebreak_hash(world, src.id, tgt.id), weighted_d)
        candidates.sort(key=_k4)
...
def _frontier_guard_reserve(world, planet):
    if not FRONTIER_GUARD_ENABLED:
        return 0
    if FRONTIER_GUARD_4P_ONLY and world.is_2p:
        return 0
    if world.step < FRONTIER_GUARD_TURN_MIN or world.step > FRONTIER_GUARD_TURN_MAX:
        return 0
    if int(planet.production) <= 0:
        return 0

    nearest_enemy = None
    for enemy in world.enemy_planets:
        if not is_targetable(world, enemy):
            continue
        d = dist(planet.x, planet.y, enemy.x, enemy.y)
        if nearest_enemy is None or d < nearest_enemy:
            nearest_enemy = d
    if nearest_enemy is None or nearest_enemy > FRONTIER_GUARD_ENEMY_DIST:
        return 0

    base = FRONTIER_GUARD_BASE + int(planet.production) * FRONTIER_GUARD_PROD_MULT
    frac = int(int(planet.ships) * FRONTIER_GUARD_FRAC)
...
def _hammer_should_fire(world):
    ...
    if not world.is_2p and world.step < HAMMER_4P_TURN_MIN:
        return False
    if world.is_late:
        return True
...
def handle_four_p_pressure_fallback(world, available, spent, target_locked, moves, mode_log):
    if not FOUR_P_PRESSURE_FALLBACK_ENABLED:
        return
    if world.is_2p or world.step < FOUR_P_PRESSURE_TURN_MIN:
        return
    if len(moves) > FOUR_P_PRESSURE_MAX_MOVES_BEFORE:
        return
    ...
    for src in world.my_planets:
        ...
        if avail >= FOUR_P_PRESSURE_MIN_AVAIL:
            sources.append((avail, src))
    if not sources:
        return
    sources.sort(reverse=True, key=lambda kv: kv[0])
    ...
    for tgt in world.planets:
        ...
        if not is_targetable(world, tgt):
            continue
        if _neutral_blocked_by_cap(world, tgt):
            continue
        if friendly_already_committed(world, tgt.id):
            continue

        owner_bonus = 0.0
        if tgt.owner == world.leader_id and world.leader_id != world.player:
            owner_bonus += FOUR_P_PRESSURE_LEADER_BONUS
        elif tgt.owner != -1:
            owner_bonus += FOUR_P_PRESSURE_ENEMY_BONUS
        else:
            owner_bonus -= FOUR_P_PRESSURE_NEUTRAL_PENALTY
        value = owner_bonus + int(tgt.production) * 7.0 - int(tgt.ships) * 0.35
        candidates.append((value, tgt))
    if not candidates:
        return
    candidates.sort(reverse=True, key=lambda kv: kv[0])

    for _avail, src in sources[:3]:
        avail = available[src.id] - spent[src.id]
        for _score, tgt in candidates[:8]:
            plan = plan_solo_capture(world, src, tgt, avail, FOUR_P_PRESSURE_MAX_TRAVEL)
            if plan is None:
                continue
            angle, turns, ships = plan
            _commit_fleet(world, moves, spent, target_locked,
                          src.id, tgt.id, angle, turns, int(ships))
            mode_log[src.id] = "4p-pressure-fallback"
            return
...
def plan_moves(world, deadline=None):
    ...
    for p in world.my_planets:
        ...
        reserve = max(reserve, _frontier_guard_reserve(world, p))
    ...
    if not _over_budget():
        handle_four_p_pressure_fallback(world, available, spent, target_locked, moves, mode_log)
