...
def line_seg_min_dist(x1: float, y1: float, x2: float, y2: float, px: float, py: float) -> float:
    ...
    # Thay vì đo khoảng cách từ mặt trời tới 2 đầu đoạn, ta cần khoảng cách tới cả đoạn bay.
    dx, dy = x2 - x1, y2 - y1
    len_sq = dx * dx + dy * dy
    if len_sq == 0:
...
def multi_leg_path(x1: float, y1: float, x2: float, y2: float, margin: float = 2.0):
    ...
    # Thử các điểm trung gian quanh mặt trời để bẻ đường bay thành 2 chặng an toàn.
    ...
    # Chọn waypoint tạo tổng quãng đường ngắn nhất.
    best_wp = None
    best_dist = float('inf')
...
def estimate_capture_bonus(src_x: float, src_y: float, planet, omega: float, ships: int) -> float:
    ...
    # Kiểm tra nhiều vị trí tương lai/quá khứ gần đó; càng nhiều vị trí bắn an toàn thì càng dễ chiếm.
    safe_count = 0
...
def agent(obs):
    ...
    - player: id của mình.
    - planets: danh sách hành tinh, mỗi phần tử thường có dạng
      [id, owner, x, y, radius, ships, production].
    - fleets: danh sách đội tàu đang bay, thường có dạng
      [id, owner, x, y, angle, from, ships].
    - angular_velocity: tốc độ quay của các hành tinh quanh mặt trời.

    Output là moves:
    - Mỗi move có dạng [src_id, angle, send].
    - src_id: hành tinh của mình sẽ gửi quân.
    - angle: góc bắn tính bằng radian.
    - send: số ship gửi đi.
    """
